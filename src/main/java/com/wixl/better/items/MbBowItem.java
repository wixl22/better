...
public class MbBowItem extends BowItem {
	...
	public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
		shootArrow(stack, world, user, remainingUseTicks, 3f);
	}

	public void shootArrow(ItemStack stack, World world, LivingEntity user, int remainingUseTicks, float power) {
		if (user instanceof PlayerEntity) {
			...
			if (!itemStack.isEmpty() || bl) {
				...
				float f = getPullProgressBetter(i);
				if ((double) f >= 0.1D) {
					...
					if (!world.isClient) {
						...
						persistentProjectileEntity.setProperties(playerEntity, playerEntity.pitch, playerEntity.yaw, 0.0F, f * power, 1.0F);
						if (f == 1.0F) {
